...
| [graphql](./graphql.md) | Apollo / auth transport | Dual fetch for Me/refresh; Apollo 4 `loading` is true during polls; partner `restaurantReservations` period/status filters use restaurant TZ; offline refresh no longer hard-signs-out; unused shared `BOOK`; sparse cache policies |
