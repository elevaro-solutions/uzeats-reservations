...
## [2026-09-14] Offline launch vs mid-request 401 diverge
- `refreshMe` keeps tokens and sets `sessionOffline` on network refresh failure. Apollo `errorLink` on `UNAUTHENTICATED` always clears tokens if refresh throws — including network failures.
- Why it matters: Cold-start offline preserves session; a flaky network during a GraphQL call can hard-sign-out.

## [2026-09-14] Sparse but load-bearing cache policies
- `Address` uses `merge: true` (no id). `bookableTables` keys on `restaurantId/slotStart/partySize` and replaces incoming.
...
- `operations.ts` exports `BOOK` (`createReservation`), but live booking uses richer `CREATE_RESERVATION` in `features/booking/api/booking.operations.ts`. Password reset hardcodes `app: "web"` in `graphql/auth.tsx`.
