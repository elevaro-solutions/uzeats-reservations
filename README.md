...
- Reviews after past visits (completed, or confirmed/seated after the slot ends) with Overall, Food, Service, and Atmosphere ratings plus up to 3 photos; diners see Leave a review on the restaurant page after a completed visit; partners reply from the dashboard Reviews page (optional Gemini draft via `GEMINI_API_KEY`) and can add diner photos to the gallery
- Menus + DO Spaces uploads (local `.data/uploads` fallback without Spaces keys; image types only)
- Notifications: email (SendGrid or Resend) with booking confirmation `.ics` attachments plus Google Calendar and View reservation links, Telegram bot, web/Expo push (service worker on diner web), in-app inbox + channel prefs; transactional SMS with public `/sms` opt-in
...
- Partner Settings hub, notifications prefs, **Reviews** (reply, Gemini draft, add diner photos to gallery), multi-restaurant selector, self-registration with Stripe card collection, **add restaurant requires a payment method** (including during a free trial), plan change preview (prorated upgrades / scheduled downgrades), onboarding checklist, **profile page** (public profile change requests), **public URL slug change requests**, toggles to accept online reservations or hide the booking widget, and **DoorDash/Uber Eats import** (saved `.mhtml`/`.html` page, including menu photos) from Settings, profile, register, and admin restaurant create
...
- Owner phone / walk-in bookings, **diner and partner reservation edit**, reservation-scoped messaging, and **individual reservation detail pages** (partner list: status + date presets; diner upcoming / past / deposit filters) plus diner **billing history** (`/billing`); bookings can attach **experiences** and private dining spaces
- Partner **My restaurants** multi-location overview alongside the home dashboard; **page search** (⌘K / Ctrl+K) jumps to partner or admin pages, settings tools, and restaurants
- Partner **Table layout** (rotatable tables, resizable canvas), **Live floor** (per-area grids, silent 30s poll), and **Tables & shifts** (tabs + modal add/edit; searchable floor areas)
- Enhanced restaurant detail pages with **logo**, photo gallery, reviews, FAQ, terms, about, **popular dishes** (up to 10), reservation windows (no timezone suffix), a Google Maps address link, and a dedicated **experience booking** flow; bookable slots use the restaurant’s local timezone
- Platform admin: **diners / restaurant owners / staff** (separate list + detail pages; create, edit, invite, venue assignment, impersonation, password reset) plus platform users, restaurants (list + **detail manage** for menu/reservations/reviews/invoices/package/booking widget/team/tables/shifts, **bulk status/delete**, **public URL slugs**), **Reservations** (all bookings), **URL slug requests**, **public profile requests**, **discovery taxonomy**, **services** catalog, invoices (**automatic monthly** plan + cover fees, manual create, branded PDF, email, public Stripe pay links), **Loyalty** (stats + super-admin program editor), exports (CSV/Excel/JSON/PDF, diner search and restaurant guest CRM), revenue, support, moderation, **TipTap email templates** with preview, config, annual billing discounts, **docs access** requests, and richer audit log filters
