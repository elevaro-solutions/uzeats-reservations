...
- Reviews after completed visits (post-visit prompt to review and save the restaurant) with Overall, Food, Service, and Atmosphere ratings
...
- Partner Settings hub, notifications prefs, multi-restaurant selector, self-registration with Stripe card collection, plan change preview (prorated upgrades / scheduled downgrades), onboarding checklist, **profile page**, **public URL slug change requests**, toggles to accept online reservations or hide the booking widget, and **DoorDash/Uber Eats import** (saved `.mhtml`/`.html` page, including menu photos) from Settings, profile, register, and admin restaurant create
...
- Enhanced restaurant detail pages with photo gallery, reviews, FAQ, terms, about, **popular dishes** (up to 10), and a dedicated **experience booking** flow; hours and bookable slots use the restaurant’s local timezone
