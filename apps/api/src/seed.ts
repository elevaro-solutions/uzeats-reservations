...
function unsplash(photoId: string, w = 800, h = 600) {
...
const CUISINE_PHOTOS: Record<string, readonly string[]> = {
  'Uzbek/Central Asian': VERIFIED_FOOD_PHOTOS,
  Mediterranean: VERIFIED_FOOD_PHOTOS,
  Italian: VERIFIED_FOOD_PHOTOS,
  Japanese: VERIFIED_FOOD_PHOTOS,
  Mexican: VERIFIED_FOOD_PHOTOS,
  Indian: VERIFIED_FOOD_PHOTOS,
  American: VERIFIED_FOOD_PHOTOS,
  Seafood: VERIFIED_FOOD_PHOTOS,
  Turkish: VERIFIED_FOOD_PHOTOS,
  Thai: VERIFIED_FOOD_PHOTOS,
};
...
const DEFAULT_PHOTOS = VERIFIED_FOOD_PHOTOS;

function restaurantPhotos(cuisine: string, index: number): string[] {
...
function menuItemPhotoUrl(itemName: string, cuisine: string) {
  const pool = CUISINE_PHOTOS[cuisine] ?? DEFAULT_PHOTOS;
...
async function seed() {
  ...
  for (const [i, r] of restaurantsData.entries()) {
    ...
    const restaurant = await Restaurant.create({
      ...
      photos: restaurantPhotos(r.cuisine, i),
      status: r.status,
      ownerId: owner!._id,
